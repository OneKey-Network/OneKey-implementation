...
const pafNodeHost = 'http://localhost';
...
describe('Function getNewId', () => {
  ...
  test('should return new ID', async () => {
    ...
    try {
      const identifier: Identifier = await getNewId({ proxyHostName: pafNodeHost });
...
describe('Function signPreferences', () => {
  test('should return fetch response', async () => {
    ...
    const result = await signPreferences({ proxyHostName: pafNodeHost }, input);
...
describe('Function createSeed', () => {
  ...
  test('with empty transmission_ids', async () => {
    const seed = await createSeed({ proxyHostName: pafNodeHost }, []);
    expect(seed).toBeUndefined();
  });

  test('with no id and preferences', async () => {
    CookiesHelpers.clearPafCookies();
    const seed = await createSeed({ proxyHostName: pafNodeHost }, transmission_ids);
    expect(seed).toBeUndefined();
  });

  test('nominal path', async () => {
    const entry = await createSeed({ proxyHostName: pafNodeHost }, transmission_ids);
