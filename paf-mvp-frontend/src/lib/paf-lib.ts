 ...
 *
 * @param hostname domain called for creating the Seed (POST /paf-proxy/v1/seed).
 ...
 */
export const generateSeed = async (hostname: string, pafTransactionIds: TransactionId[]): Promise<Seed | undefined> => {
  const entry = await createSeed({ proxyHostName: hostname }, pafTransactionIds);
 ...
 * @param pafTransactionId  Transaction Id generated for the PAF Audit Log and used for signing the Seed.
 * @param slotId
...
export const registerTransmissionResponse = (
  ...
  const seedEnty = seedStorage.get(pafTransactionId);
  if (seedEnty === undefined) {
    return undefined;
  }

  const auditLog = buildAuditLog(seedEnty.seed, seedEnty.idsAndPreferences, transmissionResponse, slotId);
