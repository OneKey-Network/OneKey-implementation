import {
  ...
  RefreshResult,
  ShowPromptOption,
} from '@frontend/lib/i-one-key-lib';
...
import { executeInQueueAsync } from '@frontend/utils/queue';
...
export class OneKeyLib implements IOneKeyLib {
   ...
   * Useful for fetching the Audit Log then.
   */
  private prebidTransactionIdByDivId = new Map<DivId, PrebidTransactionId>();

  /** Internal storage for generated Seeds. */
  seedStorage = new Map<TransactionId, SeedEntry>();

  /** Internal storage for fetching the Audit Log for a given Prebid Transaction Id. */
  auditLogByPrebidTransactionId = new Map<PrebidTransactionId, AuditLog>();
  ...
  constructor(clientHostname: string, triggerRedirectIfNeeded = true) {
  ...
  registerTransmissionResponse = (
    ...
    const seedEntry = this.seedStorage.get(pafTransactionId);
    ...
    this.auditLogByPrebidTransactionId.set(prebidTransactionId, auditLog);
    this.prebidTransactionIdByDivId.set(divId, prebidTransactionId);
  ...
  getAuditLogByTransaction = (prebidTransactionId: PrebidTransactionId): AuditLog | undefined => {
    return this.auditLogByPrebidTransactionId.get(prebidTransactionId);
  };
  ...
  getAuditLogByDivId = (divId: DivId): AuditLog | undefined => {
    const prebidTransactionId = this.prebidTransactionIdByDivId.get(divId);
    if (prebidTransactionId === undefined) {
      return undefined;
    }
    return this.getAuditLogByTransaction(prebidTransactionId);
  };
