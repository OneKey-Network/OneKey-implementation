...
export abstract class FieldReadOnly<T> implements IFieldBind {
  // List of bindings to HTML elements for the field.
  protected bindings: IBindingField<T>[] = [];
...
export abstract class Field<T> extends FieldReadOnly<T> implements IFieldReset {
  ...
  constructor(model: Model, defaultValue: T) {
    super(model);
    this.defaultValue = defaultValue;
    this._value = defaultValue;
  }

  // Called after the set value for this field has been actioned to determine if
  // other fields in the model need to be altered.
